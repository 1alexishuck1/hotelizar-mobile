...
const FormRegistro = () => {
    ...
    const handleClick = async() =>{
        ...
        const dni_client = parseInt(formData.dni_client, 10);

        if ( !( ((dni_client.toString()).length === 7 || (dni_client.toString()).length === 8) ) ) {
    ...
    <FormControl w="100%" maxW="350px" display="flex" flexDirection="column" alignItems="center" gap="15px">
        ...
        <Box w="100%">
                <InputGroup>
                    ...
                    <Input type='number' w="100%" h="40px" variant='outline' placeholder="3446-234312" color="#fff" sx={{ '::placeholder': { color: '#fff'},}} name="phone_client" onChange={handleChange} />
                </InputGroup>
        </Box>
        <Box w="100%">
            <InputGroup>
